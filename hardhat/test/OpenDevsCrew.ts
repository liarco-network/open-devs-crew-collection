...
const FIRST_BLOCK_TIMESTAMP = (new Date('October 25, 2022 10:00:00 UTC')).getTime() / 1000;
...
(async () => await time.setNextBlockTimestamp(FIRST_BLOCK_TIMESTAMP))();
...
describe('OpenDevsCrew', function () {
  ...
  async function getDeployerWallet() {
    ...
    return await ethers.provider.getSigner(DEPLOYER_WALLET_ADDRESS);
  }
  ...
  async function mainFixtures() {
    ...
    const collectionFixture = await deployCollectionFixture(deployerWallet);

    return {
      ...await impersonateAccountsFixture(collectionFixture.openDevsCrew),
      ...await deployWethFixture(),
      ...erc20TokenMockFixture,
      ...collectionFixture,
      ...await deployNonPayableContractsFixture(collectionFixture.openDevsCrew, deployerWallet),
      ...await deployPartiallyMintedCollectionFixture(deployerWallet),
      ...await deployFullyMintedCollectionFixture(deployerWallet),
      ...await deployCollectionWithFundsFixture(deployerWallet),
    };
  }

  async function notReadyToMintCollectionFixture() {
    const deployerWallet = await getDeployerWallet();
    const OpenDevsCrew = await ethers.getContractFactory('OpenDevsCrew', deployerWallet);
    const openDevsCrewNotReadyToMint = await OpenDevsCrew.deploy();

    return { openDevsCrewNotReadyToMint, deployerWallet };
  }
  ...
  async function deployCollectionFixture(deployerWallet: Signer) {
    const OpenDevsCrew = await ethers.getContractFactory('OpenDevsCrew', deployerWallet);
    const openDevsCrew = await OpenDevsCrew.deploy();

    await time.setNextBlockTimestamp(MINT_START_TIMESTAMP + 1);

    return { openDevsCrew, deployerWallet };
  }

  async function deployNonPayableContractsFixture(openDevsCrew: OpenDevsCrew, deployerWallet: Signer) {
  ...
  async function deployPartiallyMintedCollectionFixture(deployerWallet: Signer) {
    const OpenDevsCrew = await ethers.getContractFactory('OpenDevsCrew', deployerWallet);
  ...
  async function deployFullyMintedCollectionFixture(deployerWallet: Signer) {
    const OpenDevsCrew = await ethers.getContractFactory('OpenDevsCrew', deployerWallet);
  ...
  async function deployCollectionWithFundsFixture(deployerWallet: Signer) {
    const OpenDevsCrew = await ethers.getContractFactory('OpenDevsCrew', deployerWallet);
  ...
  describe('NftToken', function () {
    it('Check mint failure (too soon)', async function () {
      const { openDevsCrewNotReadyToMint: openDevsCrew } = await loadFixture(notReadyToMintCollectionFixture);

      const TOKEN_PRICE = await openDevsCrew.TOKEN_PRICE();

      expect(await time.latest()).lt(MINT_START_TIMESTAMP);

      await expect(openDevsCrew.mint(2, { value: TOKEN_PRICE.mul(2) }))
        .to.be.revertedWithCustomError(openDevsCrew, 'MintIsNotOpenYet');
    });

    it('Check mint with invalid price', async function () {
      const { openDevsCrew } = await loadFixture(mainFixtures);
  ...
  describe('Deployment', function () {
    it('Verify ownership', async function () {
      const { openDevsCrew, deployerWallet } = await loadFixture(mainFixtures);
  ...
  describe('SmartCommunityWallet', function () {
    ...
    it('Refresh wallet (sold-out)', async function () {
      const { fullyMintedCollection: openDevsCrew, deployerWallet } = await loadFixture(mainFixtures);
    ...
    it('Check events emission (receiving funds)', async function () {
      const { partiallyMintedCollection: openDevsCrew, deployerWallet } = await loadFixture(mainFixtures);
    ...
    it('Check balance of not minted token', async function () {
      const { partiallyMintedCollection: openDevsCrew } = await loadFixture(mainFixtures);
    ...
    it('Check startup funds withdraw', async function () {
      const { collectionWithFunds: openDevsCrew, mepWallet, hashLipsWallet } = await loadFixture(mainFixtures);
    ...
    it('Check startup funds full withdraw', async function () {
      const { collectionWithFunds: openDevsCrew } = await loadFixture(mainFixtures);
    ...
    it('Check events emission (startup funds)', async function () {
      const { mepWallet, hashLipsWallet, partiallyMintedCollection: openDevsCrew } = await loadFixture(mainFixtures);
    ...
    it('Check startup non payable wallet withdraw', async function () {
      const {
        openDevsCrewBrokenHashLipsWalletsMock,
        openDevsCrewMepWalletMock,
      } = await loadFixture(mainFixtures);
    ...
    it('Check withdraw with non payable wallet', async function () {
      const { openDevsCrew, nonPayableContractWalletMock } = await loadFixture(mainFixtures);
    ...
    it('Check WETH transfer', async function () {
      const { openDevsCrew, weth } = await loadFixture(mainFixtures);
    ...
    it('Check ERC20 withdraw', async function () {
      const { openDevsCrew,  erc20TokenMock } = await loadFixture(mainFixtures);
      const genericWallet = wallet6;
    ...
    it('Check ERC20 and WETH withdraw', async function () {
      const { openDevsCrew, weth, erc20TokenMock } = await loadFixture(mainFixtures);
      const genericWallet = wallet6;
      const genericWalletAddress = await genericWallet.getAddress();
    ...
    it('Check ERC20 invalid withdraw', async function () {
      const { openDevsCrew, erc20TokenMock, erc20DeployerAddress } = await loadFixture(mainFixtures);
    ...
    it('Check TokenData queries', async function () {
      const { partiallyMintedCollection, fullyMintedCollection } = await loadFixture(mainFixtures);
      type TokenData = {
        tokenId: BigNumber;
        ownerAddress: string;
